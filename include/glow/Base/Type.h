...
struct ShapeNCHW {
  ...
  static ShapeNCHW fromXYZ(llvm::ArrayRef<dim_t> shape) {
    assert(shape.size() == 3 && "Invalid 3d shape");
    return ShapeNCHW(shape[0], 1, shape[1], shape[2]);
  }

  static ShapeNCHW fromXY(llvm::ArrayRef<dim_t> shape) {
    assert(shape.size() == 2 && "Invalid 2d shape");
