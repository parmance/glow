...
static TypeRef getOutputTypeOfFusedRowwiseQuantizedSLS(
    Function *F, const llvm::ArrayRef<dim_t> &inDims,
    const llvm::ArrayRef<dim_t> &lengthsDims, ElemKind scaleOffsetKind) {
  ShapeVector outDims(inDims.begin(), inDims.end());
...
static Constant *quantizeDataForFusedRowwiseQuantizedSparseLengthsWeightedSum(
  ...
  case ElemKind::FloatTy: {
    Constant *rwqData = F->getParent()->createConstant(
        ...
        {fDims.first, fDims.second + 2 * (dim_t)sizeof(float)}, 0.0, 0, "data");
  ...
  case ElemKind::Float16Ty: {
    Constant *rwqData = F->getParent()->createConstant(
        ...
        {fDims.first, fDims.second + 2 * (dim_t)sizeof(float16_t)}, 0.0, 0,
        "data");
